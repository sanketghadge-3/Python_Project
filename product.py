...
class productClass:
    def __init__(self,root):
        ...
        txt_product_name=Entry(product_Frame,textvariable=self.var_name,font=("goudy old style",18),bg="lightyellow").place(x=150,y=160,width=200)
        txt_price=Entry(product_Frame,textvariable=self.var_price,font=("goudy old style",18),bg="lightyellow").place(x=150,y=210)
        txt_quantity=Entry(product_Frame,textvariable=self.var_qty,font=("goudy old style",18),bg="lightyellow").place(x=150,y=260)
        
        cmb_search=ttk.Combobox(product_Frame,textvariable=self.var_status,values=("Active","Inactive"),state="readonly",justify=CENTER,font=("goudy old style",15,"bold"))
        cmb_search.place(x=150,y=310,width=200)
 ...
 #================================================================   
    def fetch_cat_sup(self):
       ...
       cur=con.cursor()
       try:
         cur.execute("select name from cat13")
         cat=cur.fetchall()
         if len(cat)>0:
            del self.cat_list[:]
         ...
         cur.execute("select name from sup13")
         sup=cur.fetchall()
         if len(sup)>0:
            del self.sup_list[:]
    ...
    def add(self):
        ...
        try:
             if self.var_cat.get()=="Select" or self.var_cat.get()=="Empty"or self.var_sup.get()=="Select" or self.var_name.get()=="":
                messagebox.showerror("Error",f"  All fileds Must be required",parent=self.root)
             else:
               cur.execute("select * from pdt13 where name=?",(self.var_name.get(),))
               row =cur.fetchone()
               if row!=None:
                 messagebox.showerror("Error","This Product alredy present , try different",parent=self.root)
               else:
                cur.execute("Insert into pdt13 (Category,Supplier,name,price,qty,status) values(?,?,?,?,?,?)",(
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),
    ...
    def update(self):
        ...
        try:
             if self.var_pid.get()=="":
                messagebox.showerror("Error",f" Please select ID product from list",parent=self.root)
             else:
               cur.execute("select * from pdt13 where pid=?",(self.var_pid.get(),))
               row =cur.fetchone()
               if row==None:
                 messagebox.showerror("Error","Invalid  Product ",parent=self.root)
               else:
                cur.execute("Update  pdt13  set Category=?,Supplier=?,name=?,price=?,qty=?,status=? where pid=?",(
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),
          ...
          if self.var_pid.get()=="":
               ...
               if row==None:
                 messagebox.showerror("Error","Invalid  Product ",parent=self.root)
                  ...
                  if op==True:
                    cur.execute("delete from pdt13 where pid=?",(self.var_pid.get(),))
                    con.commit()
    ...
    def search(self):
       ...
       try:
          if self.var_searchby.get()=="Select":
             ...
             cur.execute("select * from pdt13 where "+self.var_searchby.get()+" LIKE '%"+self.var_searchtxt.get()+"%'")
             rows=cur.fetchall()
             if len(rows)!=0: 
               self.product_table.delete(*self.product_table.get_children())
               for row in rows:
