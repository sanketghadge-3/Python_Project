...
class supplierClass:
    def __init__(self,root):
        self.root = root
        self.root.geometry("1250x510+0+0")
        ...
        #=====option=====
        lbl_search=ttk.Label(self.root,text="Search By Invoice No",font=("goudy old style",15,"bold"))
        lbl_search.place(x=700,y=80)

        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15,"bold"),bg="lightyellow").place(x=900,y=80,width=160)
        btn_search=Button(self.root,text="Search",command=self.search,font=("goudy old style",15,"bold"),bg="#4caf50",fg="white",cursor="hand2").place(x=1080,y=79,width=100,height=28)

        #=====title=====
        title=Label(self.root,text="Supplier Details",font=("goudy old style",20,"bold"),bg="#0f4d7f",fg="white").place(x=50,y=10,width=1150,height=40)
        ...
        emp_frame.place(x=700,y=120,width=500,height=350)
    ...
    def add(self):
        ...
        try:
             if self.var_sup_invoice.get()=="":
                messagebox.showerror("Error",f" Invoice  Must be re quired",parent=self.root)
             else:
               cur.execute("select * from sup13 where invoice=?",(self.var_sup_invoice.get(),))
               row =cur.fetchone()
               if row!=None:
                 messagebox.showerror("Error","This Invoice No already assigned , try different",parent=self.root)
               else:
                cur.execute("Insert into sup13 (invoice,name,contact,desc) values(?,?,?,?)",(
        ...
        try:
             if self.var_sup_invoice.get()=="":
               ...
               if row==None:
                ...
                cur.execute("Update  sup13  set name=?,contact=?,desc=? where invoice=?",(
                    self.var_name.get(),
                    self.var_contact.get(),
                    self.txt_desc.get('1.0',END),
                    self.var_sup_invoice.get(),
          ...
          if self.var_sup_invoice.get()=="":
               ...
               if row==None:
                 messagebox.showerror("Error","Invalid Invoice No ",parent=self.root)
                  ...
                  if op==True:
                    cur.execute("delete from sup13 where invoice=?",(self.var_sup_invoice.get(),))
                    con.commit()
    ...
    def search(self):
       ...
       try:
          if self.var_searchtxt=="":
             ...
             cur.execute("select * from sup13 where invoice=?",(self.var_searchtxt.get(),))
             row=cur.fetchone()
             if row!=None: 
               self.SupplierTable.delete(*self.SupplierTable.get_children())
               self.SupplierTable.insert('',END,values=row)
